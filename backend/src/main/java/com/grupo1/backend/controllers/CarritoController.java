...
public class CarritoController {
        ...
        try {
            if (carrito == null) {
                return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                        .body("El carrito puede estar vacio pero no puede ser nulo");
            }
    ...
    public ResponseEntity<?> addProductos(@PathVariable int idCarrito, @RequestBody Producto producto) {
        Optional<Carrito> carritoOpt = carritoSer.getById(idCarrito);
        ...
        Carrito carritoActualizado = carritoSer.addCarrito(carrito);
    ...
    public ResponseEntity<?> getByUser(@PathVariable int id_user) {
        System.out.println("userGet" + id_user);
        try {
            if (id_user <= 0) {
